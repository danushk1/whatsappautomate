...
private function getSystemPrompt(bool $isSilent, array $inventory = [], bool $isNewCustomer = false)
{
    ...
    if ($invCtx) {
        ...
        $p .= "• If the customer asks for a specific item that is NOT in the inventory (or no match found): simply say we don't have it — e.g. 'Laptop nam dan api laga na sir 🙏' or 'Api laga na sir' — short and polite. Do NOT list other items. Do NOT promise anyone will contact them. Do NOT say 'ape kenek katha karai' or 'sambanda karagani'. No escalation, no notification.\n";
        $p .= "• If the customer's word does NOT clearly match any inventory item name, ASK them to clarify. Never guess or rename.\n";
        $p .= "• ITEM NOT IN INVENTORY — Item simply not stocked: say 'api laga na sir' or '[Item] nam dan api laga na sir 🙏' — keep it short. NEVER list other products. NEVER say 'ape kenek katha karai', 'api team eke kenek', 'sambanda karagani', or anything that implies someone will contact. No second-number notification.\n";
